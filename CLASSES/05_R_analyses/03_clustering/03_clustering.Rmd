...
# for instance, you can pick the first two dbps (1:2) and then calculate the distance between them using the accurate
# statistic. Binary (if between 0 and 1) or euclidean (betwee 1 to 10,000) and everything in-between.
...
# we now can plot the information in bin_hier (binary hierarchical clustering)
plot(bin_hier)
...
# Note we can't use ggsave here :( (because we didn't use ggplot/ggdendro to make the plots) but this is a great way to save
# pdfs for all plots.
