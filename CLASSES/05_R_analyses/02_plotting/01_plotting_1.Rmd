...
# Wow that sure looks different :) of note the inflection point
...
# scale_x_continous (same for y) is calling the scale layer specifically
...
# scales here, just allows the chart to create its custom axes limits based on the data
```
...
# the values emanate from the center and are a "bar_plot" from the center.
...
ggplot(big_num_peaks_df, aes(x = num_peaks)) +
  geom_density(alpha = 0.5, color = "#424242", fill = "#424242") +
  theme_paperwhite() +
