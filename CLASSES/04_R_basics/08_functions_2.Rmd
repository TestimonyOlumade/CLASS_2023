...
# This will take a minute because it will pull out every peak file from the file path, importing them as a Genomic ranges, 
# and attach the names to the files, so that they can be tracked

# quick check of peak numbers per file
num_peaks <- sapply(peak_list, length) 
num_peaks

# This is important to be done - to take a look at the length of each peak file so as to see the level of variations across
# replicates. If variations are too low on any of them, they may be excluded so that they do not bias analysis.
...
# we need a unique DBP (DNA Binding Protein) name with out the replicate as input to for loop
names(peak_list)

# This is important - to remove the replicate number at the name of the peak name so that we can have just the DBP name 
# and then we can find all the peak overlaps - to return with the peaks that are common to all replicates, under one name.
...
dbps
  ...
  # If you need the peaks to be in more than 2 replicates, then you replace '1' with '2'
}
```
